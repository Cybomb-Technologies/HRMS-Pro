  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/offboarding/${employeeId}/documents`,
        {
          method: "POST",
          body: formData,
        }
  ...
  return (
    <DialogContent className="max-w-2xl">
      <DialogHeader>
        ...
        <DialogDescription>
          Upload required documents for this offboarding step. Supported
      ...
      {selectedEmployee && (
        <div className="p-4 bg-gradient-to-r from-red-50 to-pink-50 rounded-lg border border-red-200">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div className="flex items-center gap-2">
              <span className="text-red-700 font-medium min-w-16">
                Department:
              </span>
              <span className="text-gray-900">
              ...
              <Badge
                className={
                  selectedEmployee.status === "active"
                    ? "bg-green-100 text-green-800"
                    : selectedEmployee.status === "on-notice"
                    ? "bg-yellow-100 text-yellow-800"
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="text-center p-8 border rounded-lg bg-gray-50">
            ...
            <p className="text-gray-500 mb-4">
              Please add employees first before starting offboarding
            </p>
            <Button
              type="button"
              variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="lastWorkingDay" className="text-sm font-medium">
            Last Working Day *
          </Label>
          <Input
            id="lastWorkingDay"
          ...
          <p className="text-xs text-gray-500">
            ...
            {formData.lastWorkingDay
              ? new Date(formData.lastWorkingDay).toLocaleDateString("en-US", {
                  weekday: "long",
                  year: "numeric",
                  month: "long",
                  day: "numeric",
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="Payroll Department">
                Payroll Department
              </SelectItem>
            </SelectContent>
          </Select>
...
const StepCompletionSection = ({ employee, onStepUpdate }) => {
  ...
  const handleCompleteStep = async (step) => {
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/api/offboarding/${employee.employeeId}/step`,
        {
          method: "PUT",
  ...
  const handleDeleteDocument = async (stepId, documentId) => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/offboarding/${employee.employeeId}/documents/${documentId}`,
        {
          method: "DELETE",
        }
      );
      ...
      const offboardingResponse = await fetch(
        `http://localhost:5000/api/offboarding/${employee.employeeId}`
      );
  ...
  const getStepIcon = (stepName) => {
    switch (stepName) {
      ...
      case "Knowledge Handover":
        return User;
      case "Final Timesheet":
        return Clock;
      case "Leave Encashment":
        return Calendar;
      case "F&F Calculation":
        return DollarSign;
      case "Profile Deactivation":
        return Shield;
      default:
        return FileText;
    }
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {Math.round(employee.progress)}% complete
              </p>
            </div>
            <div>
              <Label className="text-sm font-medium text-gray-700">
...
const OffboardingSection = () => {
  ...
  // ✅ FIXED: Send reminder notification to employee
  const handleSendReminder = async (employee) => {
    try {
      console.log(
        "🔔 [DEBUG] Sending offboarding reminder to:",
        employee.employeeId
      );
      ...
      const response = await fetch(
        "http://localhost:5000/api/notifications/offboarding/reminder",
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to send reminder notification"
        );
      }
      ...
      toast({
        title: "Reminder Sent",
        description: `A reminder has been sent to ${employee.name} for their offboarding process.`,
      });
    } catch (err) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (offRes.ok) {
          ...
          if (viewingEmployee && viewingEmployee.employeeId === employeeId)
  ...
  const handleStepUpdate = (updatedOffboarding) => {
    ...
    if (
      viewingEmployee &&
      viewingEmployee.employeeId === updatedOffboarding.employeeId
    ) {
  ...
  const handleGenerateFF = async (employee) => {
    try {
      const res = await fetch(
        `http://localhost:5000/api/offboarding/${employee.employeeId}/settlement`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleCompleteOffboarding = async (employee) => {
    ...
    const ok = window.confirm(
      `Are you sure you want to COMPLETE offboarding for ${employee.name}? This will mark the employee as inactive.`
    );
    if (!ok) return;
    ...
    try {
      toast({
        title: "Completing...",
        description: `Completing offboarding for ${employee.name}...`,
      });

      const res = await fetch(
        `http://localhost:5000/api/offboarding/${employee.employeeId}/complete`,
        {
          method: "PUT",
        }
      );
      ...
      if (!res.ok) {
        ...
        throw new Error(
          err.error || err.message || "Failed to complete offboarding"
        );
      }

      // ✅ NEW: Send completion notification to employee
      try {
        ...
        const notificationResponse = await fetch(
          "http://localhost:5000/api/notifications/offboarding/completion",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (notificationResponse.ok) {
          console.log(
            "✅ [DEBUG] Offboarding completion notification sent successfully"
          );
        } else {
          console.error(
            "❌ [DEBUG] Failed to send offboarding completion notification"
          );
        }
      } catch (notificationError) {
        console.error(
          "❌ [DEBUG] Error sending offboarding completion notification:",
          notificationError
        );
      }
      ...
      toast({
        ...
        description: `${employee.name} marked as inactive successfully and notification sent.`,
      });
    ...
    if (completedOffboarding.length === 0) {
      return (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">
            Completed offboarding processes will appear here
          </p>
        </div>
      );
    }
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-900">
                      Completed:{" "}
                      {new Date(
                        employee.updatedAt || employee.lastWorkingDay
                      ).toLocaleDateString()}
                    </p>
                    <p className="text-xs text-gray-500">
                      By: {employee.assignedTo}
                    </p>
  ...
  const tabs = [
    ...
    { id: "steps", label: "Offboarding Steps", icon: FileText },
    {
      ...
      label: "Completed",
      icon: CheckCircle,
      count: completedOffboarding.length,
    },
  ];
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Streamline employee offboarding with automated workflows, asset recovery, and final settlement calculations in HRMS Pro"
        />
      </Helmet>
      ...
      <Dialog open={isModalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Select an employee and fill in the details to begin the
              offboarding process
            </DialogDescription>
          </DialogHeader>
          <OffboardingForm
      ...
      <Dialog
        ...
        onOpenChange={() => setViewingEmployee(null)}
      >
        {viewingEmployee && (
        ...
        >
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Streamline employee exits with automated workflows, asset
          ...
          <Card className="p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {
                    offboardingEmployees.filter(
                      (c) => c.status === "pending-final"
                    ).length
                  }
                </p>
              </div>
